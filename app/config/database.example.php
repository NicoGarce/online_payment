...
function initializeDatabase() {
    ...
    // Create users table — includes employee_number for secure login
    $usersTable = "
    ...
    try {
        ...
        if ($result['count'] == 0) {
            // Create super admin account — default employee_number 11745
            $superAdminPassword = password_hash('SuperAdmin@123', PASSWORD_DEFAULT);
            $stmt = $pdo->prepare("
                INSERT INTO users (username, email, employee_number, password, first_name, last_name, role) 
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute(['web-admin', 'web-admin@uphsl.edu.ph', '11745', $superAdminPassword, 'Web', 'Administrator', 'super_admin']);
